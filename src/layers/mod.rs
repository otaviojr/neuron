  ...
  pub fn new(n_channels: usize, n_filters: usize, filter_size: usize, config: ConvLayerConfig) -> Self {
    ...
    for i in 0 .. n_filters {
      ...
      for j in 0 .. n_channels {
        let filter_channels = Tensor::random(filter_size, filter_size);
      }
  ...
  fn forward(&mut self, input: &Vec<Box<Tensor>>) -> Option<Vec<Box<Tensor>>> {
    ...
    for (f,b) in self.filters.iter().zip(self.bias.iter()) {
      ...
      for (i,fc) in input.iter().zip(f.iter()) {
        ...
        while y + self.config.filter_size < i.rows() {
          while x + self.config.filter_size < i.cols() {
            let mut sum = 0.0;
            for y1 in 0 .. self.config.filter_size {
              for x1 in 0 .. self.config.filter_size {
