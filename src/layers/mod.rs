      ...
      if let Some(ref forward_input) = self.last_input {
        for ((f,inp), b) in self.filters.iter_mut().zip(input.iter()).zip(self.bias.iter()) {
          ...
          for (fi,fc) in forward_input.iter().zip(f.iter_mut()) {            
            ...
            for i in 0..inp.rows()-self.filter_size.0 {
              for j in 0 .. inp.cols()-self.filter_size.1 {
                for k in 0 .. self.filter_size.0 {
                  for l in 0 .. self.filter_size.1 {
                    output.set(i,j,output.get(i,j) + fc.get(k,l) * inp.get(i+k,j+l));
                    dw.set(k,l,dw.get(k,l) + inp.get(i+k, j+l));
                  }
                }
                db += output.get(i,j);
              }
...
impl LayerPropagation for PoolingLayer {
  fn forward(&mut self, input: &Vec<Box<Tensor>>) -> Option<Vec<Box<Tensor>>> {
    ...
    println!("PoolingLayer Output size (Forward) = {}x{}x{}", output[0].rows(), output[0].cols(), output.len());
